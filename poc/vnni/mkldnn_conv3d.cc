...
void simple_net_int8(int batch, int ic, int h, int w, int oc, int kh, int kw, int sh, int sw) {
    ...
    // strides: {1, 1}
    int d = 16;
    memory::dims conv_src_tz = {batch, ic, d, h, w};
    memory::dims conv_weights_tz = {oc, ic, kh, kh, kw};
    memory::dims conv_bias_tz = {oc};
    memory::dims conv_dst_tz = {batch, oc, (d - kh) / sh + 1, (h - kh) / sh + 1, (w - kw) / sw + 1};
    memory::dims conv_strides = {sh, sh, sw};
    memory::dims conv_padding = {0, 0, 0};
    //[Configure tensor shapes]
    ...
    std::vector<float> user_src(batch * ic * 16 * h * w);
    std::vector<float> user_dst(batch * oc * (16 - kh + 1) * (h - kh + 1) * (w - kw + 1));
    ...
    auto user_src_memory = memory(
            {{conv_src_tz}, dt::f32, tag::ncdhw}, cpu_engine, user_src.data());
    auto user_weights_memory = memory({{conv_weights_tz}, dt::f32, tag::oidhw},
    ...
    int64_t exec_span = end_roi();
    int64_t exec_ops = (int64_t) ic * oc * (h - kh + 1) * (w - kw + 1) * kh * kw / 64;
    std::cout << exec_ops << std::endl;
    std::cout << "Exec time: " << exec_span / 1e6 << std::endl;
    std::cout << exec_ops / (exec_span / 1e6) / 1e9 << " GVNNI/s" << std::endl;
    ...
    auto user_dst_memory = memory(
            {{conv_dst_tz}, dt::f32, tag::ncdhw}, cpu_engine, user_dst.data());
